...
# Plotting scatter points for each continent with different markers and opacity encoding for population
for continent, marker in markers.items():
    ...
    plt.scatter(
        ...
        color='blue',  
        edgecolor='black',
        alpha=norm_pop[continent_data.index],  # Opacity encoding based on population
        label=continent
    )
...
# Find the top 5 countries with the smallest populations
top_5_least_population = data_2002.nsmallest(5, 'pop')

# Label the top 5 countries by the smallest population with a border
for idx, row in top_5_least_population.iterrows():
    plt.scatter(
        row['gdpPercap'], 
        row['lifeExp'], 
        color='blue', 
        marker=markers[row['continent']],  
        s=200, 
        edgecolor='black', 
        linewidth=3.5,  
        alpha=norm_pop[idx]  
...
plt.title('GDP Per Capita vs Life Expectancy (2002) - Population Encoded by Opacity', fontsize=16, fontweight='bold')
